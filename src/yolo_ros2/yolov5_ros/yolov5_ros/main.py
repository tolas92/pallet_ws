...
from yolov5_ros.utils.general import (LOGGER, check_img_size, check_imshow, non_max_suppression, scale_boxes, xyxy2xywh)
from yolov5_ros.utils.plots import Annotator, colors
from yolov5_ros.utils.torch_utils import select_device, time_sync

from yolov5_ros.utils.dataloaders import letterbox
        ...
        for i, det in enumerate(pred):
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if 'cargo_empty' in class_list:
                        self.cargo_empty_counter += 1
                    else:
                        self.cargo_empty_counter = 0
                        
                    if self.cargo_empty_counter > 10:
                        screenshot_filename = 'cargo_empty.png'
                        cv2.imwrite(screenshot_filename, im0)
                        print(f"Screenshot saved as {screenshot_filename}")
                        screenshot = cv2.imread(screenshot_filename)
                        cv2.imshow('Screenshot', screenshot)
                        self.cargo_empty_counter = 0  # Reset counter after saving screenshot
                    
                    if 'cargo_25' in class_list:
                        self.cargo_25_counter += 1
                    else:
                        self.cargo_25_counter = 0

                    if self.cargo_25_counter > 10:
                        screenshot_filename='cargo_25.png'
                        cv2.imwrite(screenshot_filename,im0)
                        print(f"Screenshot saved as {screenshot_filename}")
                        screenshot = cv2.imread(screenshot_filename)
                        cv2.imshow('Screenshot', screenshot)
...
class yolov5_ros(Node):
    def __init__(self):
        ...
        self.declare_parameter('weights', str(ROOT) + '/config/truck2.pt')
        self.declare_parameter('data', str(ROOT) + '/data/truck2.yaml')
        self.declare_parameter('imagez_height', 640)
        self.declare_parameter('imagez_width', 640)
        self.declare_parameter('conf_thres', 0.70)
